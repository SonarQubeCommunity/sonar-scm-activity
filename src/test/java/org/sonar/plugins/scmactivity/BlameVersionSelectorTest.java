...
public class BlameVersionSelectorTest {
  ...
  public void should_save_blame_when_file_changes() throws IOException {
    ...
    when(fileToResource.toResource(changedSource, Arrays.asList(sourceDir), false, context)).thenReturn(resource);
    when(blameSensor.save(changedSource, resource, 3)).thenReturn(saveBlame);

    MeasureUpdate update = blameVersionSelector.select(changedSource, context, Arrays.asList(changedSource), Arrays.asList(sourceDir), false);
  ...
  public void should_copy_previous_measures_when_file_is_the_same() throws IOException {
    ...
    when(fileToResource.toResource(source, Arrays.asList(sourceDir), false, context)).thenReturn(resource);

    MeasureUpdate update = blameVersionSelector.select(source, context, Arrays.asList(changedSource), Arrays.asList(sourceDir), false);
